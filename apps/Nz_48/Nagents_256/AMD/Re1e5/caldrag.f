...
cc      real*8, dimension(num_turbine_vawt,N_ang,Nx,Ny):: ker_u,ker_w	  
     ...
     +       SUMQ,SUMb1,SUMCTd,SUMQ1,SUMCTU,Power,Power1,SUMFy,
     +	     SUMCT1,SUMCTU1,SUMCTd1,SUMdA1,SUMFx1,SUMFy1,
...
!c     +          (+CTCL*dsin(CTphi)-CTCD*dcos(CTphi))	 
      ...
      SUMCT(ii) =SUMCT(ii)+CTFx/(0.5d0*((CTU/u_star)**2.)*	      
     +           dA/(dx*dy*dz))*dA/(2.*wtR_vawt(ii)*wtH_vawt(ii))
      else
     ...
     +         (z_i**3)		
      ...
      SUMFy(ii)=SUMFy(ii)+CTFy*(dx*dy*dz)*(z_i**2.)	  
...
c      end if
     ...
     +        MPI_DOUBLE_PRECISION,0,me,nall, ierr )	
         call MPI_SEND(SUMFy,num_turbine_vawt,
     +        MPI_DOUBLE_PRECISION,0,me,nall, ierr )	 
     ...
     +        MPI_DOUBLE_PRECISION,i,i,nall,status2,ierr )	
         call MPI_RECV(SUMFy1,num_turbine_vawt,
     +        MPI_DOUBLE_PRECISION,i,i,nall,status2,ierr )		 
...
!c     +   ,SUMFx/(0.5*SUMdA*SUMCTU**2.0)
     +    ,SUMFx/(0.5*SUMdA*26./(0.75*3.)*7.04**2.0)/1.0d0
     +    ,SUMFy/(0.5*SUMdA*26./(0.75*3.)*7.04**2.0)/1.0d0	 
!c     +   ,Power/(0.5*SUMdA*SUMCTU**3.0)
     +    ,Power/(0.5*SUMdA*26./(0.75*3.)*7.04**3.0)/1.0d0
     +    ,SUMFr/(0.5*SUMdA*7.04**2.0)/1.0d0
     +    ,SUMFt/(0.5*SUMdA*7.04**2.0)/1.0d0
     +    ,SUMRe/10**6,SUMat,(2.0*wtR_vawt*z_i*wtH_vawt*z_i)	 
!      write(*,*) wtR_vawt*z_i,wtH_vawt*z_i,z_i
	  !SUMdA/(2*pi*wtR(1)*dz)/num_turbine
      end if	  
